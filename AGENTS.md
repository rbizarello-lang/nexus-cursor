...
| `src/edition-claude.jsx` | Edição Claude · Ardósia (menu, barra, Hoje, Intimações, Carteira, Visão geral, Linha do tempo, Mesa de prazos). Concatenado antes de `app.jsx` |
...
- **Claude · Ardósia**: `uiEdition: 'claude'` — menu lateral com operações, Hoje, Intimações, Carteira, Visão geral da operação (aba `visao`, só nesta edição), Linha do tempo (`viewMode: 'cx_timeline'`) e Mesa de prazos extintivos novos; demais telas vêm do app dentro da casca. Ativar via ⚙ ou `?edition=claude`. Código em `src/edition-claude.jsx`.
- A Beta e a Ardósia **não** substituem o clássico. Modelo de dados, parsers, sync Drive, formulários e calculadora de prescrição devem permanecer compatíveis entre edições, salvo pedido explícito.
