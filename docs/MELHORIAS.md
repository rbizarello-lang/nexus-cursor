...
- **Proposta (25/09/2026, não implementada):** Briefing como dossiê (leitura da operação pelas entradas fixadas, frentes processuais em trilhas horizontais, diário, coluna de apoio), Processos e prescrição como árvore única com ficha lateral (contagens da prescrição empilhadas, barra de lote no pé, agrupar por prescrição) e Relatório de passagem de serviço em camadas (capa com próximos 15 dias e alertas, frentes, anexos; modelos Resumo e Prestação de contas). Revisada com a avaliação do Fable: tese = Estratégia fixada mais recente (uma em destaque); régua só com fases registradas; Tarefas fora do Briefing; Processos começa pela ficha lateral sobre a lista atual (8a) e só depois a árvore (8b, risco alto); relatório 9a só com o modelo de passagem de serviço e a regra dos próximos 15 dias em `src/lib`. Ordem: 9a → 9b → 7a → 7b → 8a → 8b, sem mudar modelo de dados nem prescrição. Aberta: lembretes no relatório. Maquete e decisões pendentes: `design/mockups/prumo-briefing-processos-relatorio.html`. Mesa de prazos fica para outro momento.
