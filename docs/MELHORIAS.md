...
## 🧭 Nexus Prumo (antes “Claude · Ardósia”) — protótipo (✅ 23/09), Fases 1 a 7 no app (✅ 25/09/2026)
...
- Maquete: `design/mockups/prumo-briefing-processos-relatorio.html` (seção `#briefing`). O redesign da aba Processos e prescrição (sumário fixo, filtros pelos sinais da Beta, barra de grupos, tabelas agrupadas, ficha lateral — seção `#processos` da maquete) não fazia parte do pedido desta fase e continua não implementado. Para depois: histórico além dos 500 registros do `changeLog`; Mesa de prazos.
