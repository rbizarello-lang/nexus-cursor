...
- **Proposta (25/09/2026, não implementada):** Briefing como dossiê (leitura da operação pelas entradas fixadas, frentes processuais em trilhas horizontais, diário, coluna de apoio), Processos e prescrição como árvore única com ficha lateral (contagens da prescrição empilhadas, barra de lote no pé, agrupar por prescrição) e Relatório de passagem de serviço em camadas (capa com próximos 15 dias e alertas, frentes, anexos; modelos Resumo e Prestação de contas). Fases sugeridas 7a/7b (Briefing), 8a/8b (Processos), 9a/9b (Relatório), sem mudar modelo de dados nem prescrição. Maquete e decisões pendentes: `design/mockups/prumo-briefing-processos-relatorio.html`. Mesa de prazos fica para outro momento.
